...
export default function FloatingImageTool({ isOpen, onClose, onInsert }: FloatingImageToolProps) {
  ...
  const [activeTab, setActiveTab] = useState<'upload' | 'url' | 'local'>('local'); // 默认本地上传
  ...
  // 🌟 本地上传：保存到本地 public/uploads，无需图床
  const handleLocalUpload = async (file: File) => {
    setIsUploading(true);
    showToast("正在保存到本地...", "info");
    ...
    try {
      ...
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/picbed/upload_local`, {
        method: 'POST',
        body: uploadData,
      });
            ...
            {!uploadedUrl && (
              <div className="flex bg-slate-200/50 dark:bg-slate-800/50 p-1 rounded-2xl mb-5">
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all ${activeTab === 'local' ? 'bg-white dark:bg-slate-700 text-emerald-500 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                >
                  本地上传
                </button>
                <button
              ...
              activeTab === 'url' ? (
                ...
                >
                  <input type="file" ref={fileInputRef} onChange={e => {
                    if (e.target.files) {
                      if (activeTab === 'local') handleLocalUpload(e.target.files[0]);
                      else handleFileUpload(e.target.files[0]);
                    }
                  }} accept="image/*" className="hidden" />
                  <div className="text-4xl drop-shadow-sm">{isUploading ? '⏳' : '📥'}</div>
                  <div className="text-center">
                    <p className="text-sm font-bold text-slate-700 dark:text-slate-300">{isUploading ? '正在保存...' : activeTab === 'local' ? '点击或拖拽图片（本地）' : '点击或拖拽图片（云端）'}</p>
                    {activeTab === 'local' && <p className="text-[10px] text-slate-400 mt-1">无需配置图床，存到本地</p>}
                  </div>
                </div>
              )
            ) : (
