      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div key={index} className="flex justify-between items-center p-3 bg-white/40 dark:bg-slate-800/40 rounded-2xl border border-white/20 group">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      ...
                      <span className="text-[10px] font-mono text-pink-500 mt-0.5">{id.includes('|') ? '🎵 #' + id.split('|')[0] : (id.startsWith('http') ? '🔗 ' + decodeURIComponent(id.split('/').pop() || id) : '#' + id)}</span>
                    </div>
                  </div>
        ...
        <div className="bg-slate-100/50 dark:bg-slate-800/50 rounded-3xl p-6 space-y-6">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="网易云ID 或 在线mp3 或 ID|播放地址" value={formData.newMusicId} onChange={e => handleUpdate('newMusicId', e.target.value)} className="flex-1 bg-white dark:bg-slate-900 border-none rounded-2xl px-4 py-3 text-sm outline-none shadow-sm" />
