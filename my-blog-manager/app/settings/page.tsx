...
function SettingsContent() {
  ...
  const fetchMusicDetail = async (id: string) => {
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const configData = await configRes.json();
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/music/query?id=${encodeURIComponent(id)}`, { cache: 'no-store' });
      const data = await res.json();
  ...
  const syncToFrontend = async () => {
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const configData = await configRes.json();
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/sync/quick`, { method: 'POST' });
      const data = await res.json();
      if (data.success) {
  ...
  const menuItems = [
    ...
    // { id: 'aicat', name: 'AI 煤球配置', icon: '🐾' }, // 👈 已禁用 AI 煤球
        ...
        <main className="w-[95%] max-w-7xl mx-auto mt-24 flex flex-col md:flex-row gap-8 items-start relative z-10">
          ...
          <div className="w-full md:w-72 shrink-0 flex flex-col gap-4">
            ...
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-3xl p-4 mt-4">
              ...
              <button className="w-full py-2 bg-amber-500/20 text-amber-700 dark:text-amber-300 rounded-xl text-xs font-bold hover:bg-amber-500 hover:text-white transition-all text-left px-4 flex justify-between mb-2">
                <span>拉取 my-blog 数据</span><span>📥</span>
              </button>
              <button onClick={syncToFrontend} className="w-full py-2 bg-green-500/20 text-green-700 dark:text-green-300 rounded-xl text-xs font-bold hover:bg-green-500 hover:text-white transition-all text-left px-4 flex justify-between">
                <span>一键同步到前台</span><span>🚀</span>
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 w-full">
            <AnimatePresence mode="wait">
              ...
              {/* 👇 🌟 已禁用 AI 煤球 {activeTab === 'aicat' && <AICatSection key="aicat" formData={formData} handleUpdate={handleUpdate} pushToQueue={pushToQueue} />} */}
