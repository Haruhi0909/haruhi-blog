...
def get_frontend_config_path():
    """获取前台博客 XHBlogs 的 siteConfig.ts 路径"""
    # PROJECT_ROOT 是 my-blog-manager，上级目录是 XinghuisamaBlogs-main
    parent_dir = os.path.dirname(PROJECT_ROOT)
    frontend_path = os.path.join(parent_dir, 'XHBlogs', 'siteConfig.ts')
...
def update_site_config(payload: Dict[str, Any] = Body(...)):
    ...
    admin: {admin_str},
        ...
        # 🌟 同步写入前台博客 XHBlogs 的 siteConfig.ts
        frontend_path = get_frontend_config_path()
        if frontend_path:
            try:
                with open(frontend_path, 'w', encoding='utf-8') as f:
                    f.write(content)
                print(f"  ✅ 已同步到前台博客配置: {frontend_path}")
            except Exception as e:
                print(f"  ⚠️ 同步前台配置失败: {str(e)}")

        print(f"🔥 任务圆满完成，共刷新 {updated_count} 个字段")
        print("=" * 50 + "\n")

        return {"success": True, "message": f"本地配置修改成功！已同步 {updated_count} 个字段到前台和后台"}
