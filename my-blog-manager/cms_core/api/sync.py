...
async def quick_sync():
    """快速同步到默认前台目录（../XHBlogs）"""
    default_path = os.path.abspath(os.path.join(PROJECT_ROOT, "..", "XHBlogs"))
    if not os.path.exists(default_path):
        return {"success": False, "message": f"默认前台目录不存在: {default_path}"}
    try:
        # 1. 同步文件夹
        for d in SYNC_DIRS:
            src_dir = os.path.join(PROJECT_ROOT, d)
            dst_dir = os.path.join(default_path, d)
            if os.path.exists(src_dir):
                if os.path.exists(dst_dir):
                    shutil.rmtree(dst_dir)
                shutil.copytree(src_dir, dst_dir)
        ...
        for f in SYNC_FILES:
            src_file = os.path.join(PROJECT_ROOT, f.replace("/", os.sep))
            dst_file = os.path.join(default_path, f.replace("/", os.sep))
            if os.path.exists(src_file):
                os.makedirs(os.path.dirname(dst_file), exist_ok=True)
                if f == "siteConfig.ts":
                    with open(src_file, "r", encoding="utf-8") as file_in:
                    ...
                    shutil.copy2(src_file, dst_file)
        return {"success": True, "message": "🎉 已同步所有内容到前台博客！"}
    except Exception as e:
